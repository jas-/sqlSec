...
CREATE DEFINER='[dbUser]'@'[dbHost]' PROCEDURE KR_Main(IN backup BOOLEAN, IN debug BOOLEAN)
 DETERMINISTIC
 MODIFIES SQL DATA
...
CREATE DEFINER='[dbUser]'@'[dbHost]' PROCEDURE KR_RP(IN Secret LONGTEXT, IN debug BOOLEAN)
 DETERMINISTIC
 MODIFIES SQL DATA
 SQL SECURITY INVOKER
...
CREATE DEFINER='[dbUser]'@'[dbHost]' PROCEDURE KR_BU_New(IN Secret LONGTEXT)
 DETERMINISTIC
 MODIFIES SQL DATA
 SQL SECURITY INVOKER
...
CREATE DEFINER='[dbUser]'@'[dbHost]' PROCEDURE KR_New(IN t CHAR(16), IN f CHAR(32), IN s LONGTEXT)
 DETERMINISTIC
 MODIFIES SQL DATA
 SQL SECURITY INVOKER
...
CREATE DEFINER='[dbUser]'@'[dbHost]' PROCEDURE KR_CV(IN t CHAR(16), IN f CHAR(32), IN Secret LONGTEXT, IN rnd CHAR(128))
 DETERMINISTIC
 MODIFIES SQL DATA
 SQL SECURITY INVOKER
...
CREATE DEFINER='[dbUser]'@'[dbHost]' PROCEDURE KR_DV(IN rnd CHAR(64))
 DETERMINISTIC
 MODIFIES SQL DATA
 SQL SECURITY INVOKER
 COMMENT 'Drops view'
...
CREATE DEFINER='[dbUser]'@'[dbHost]' PROCEDURE KR_PT(IN rnd CHAR(128))
 DETERMINISTIC
 MODIFIES SQL DATA
 SQL SECURITY INVOKER
...
CREATE DEFINER='[dbUser]'@'[dbHost]' PROCEDURE KR_GT()
 DETERMINISTIC
 MODIFIES SQL DATA
 SQL SECURITY INVOKER
...
CREATE DEFINER='[dbUser]'@'[dbHost]' PROCEDURE KR_DT(IN tbl CHAR(16))
 DETERMINISTIC
 MODIFIES SQL DATA
 SQL SECURITY INVOKER
...
CREATE DEFINER='[dbUser]'@'[dbHost]' PROCEDURE KR_GK(OUT OutSecret CHAR(128))
 DETERMINISTIC
 SQL SECURITY INVOKER
...
CREATE DEFINER='[dbUser]'@'[dbHost]' PROCEDURE KR_SV(IN Secret LONGTEXT)
 DETERMINISTIC
 MODIFIES SQL DATA
 SQL SECURITY INVOKER
 COMMENT 'Saves newly generated encryption key'
BEGIN
 SET @sql = CONCAT('INSERT INTO `sqlSec_settings` (`version`, `epoch`, `keyID`) VALUE ("[dbVer]", UNIX_TIMESTAMP(NOW()), "',Secret,'") ON DUPLICATE KEY UPDATE `keyID`="',Secret,'"');
 PREPARE stmt FROM @sql;
 EXECUTE stmt;
 DEALLOCATE PREPARE stmt;
...
CREATE DEFINER='[dbUser]'@'[dbHost]' FUNCTION KR_GS() RETURNS varchar(64) CHARSET utf8
 DETERMINISTIC
 SQL SECURITY INVOKER
...
CREATE DEFINER='[dbUser]'@'[dbHost]' PROCEDURE KR_BU(IN Secret LONGTEXT)
 DETERMINISTIC
 MODIFIES SQL DATA
 SQL SECURITY INVOKER
...
BEGIN
 ...
 IF (Secret IS NOT NULL) THEN
  SET @sql = CONCAT('SELECT "',Secret,'" INTO OUTFILE "[dbPath]/BACKUP-',UNIX_TIMESTAMP(NOW()),'-SECRET.sql"');
  PREPARE stmt FROM @sql;
  EXECUTE stmt;
  DEALLOCATE PREPARE stmt;
 END IF;
 ...
 LOOP1: loop
  ...
  SET fName = CONCAT('[dbPath]/BACKUP-',UNIX_TIMESTAMP(NOW()),'-',t,'.sql');
